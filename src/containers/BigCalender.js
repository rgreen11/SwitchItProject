...
onEventDrop=({ event, start, end }) =>{
    ...
    axios({
      ...
      .then(data=>{
        this.setState({events: nextEvents, stamp: start})
      })
      .catch(error=>{
          console.log(error)
      })
...
componentDidMount(){
    ...
    // const storage = localStorage.getItem('ootd')

    
}

render() {
    const { events, stamp } = this.state;
    console.log(stamp)
    return (
        <div>
            <DragAndDropCalendar
